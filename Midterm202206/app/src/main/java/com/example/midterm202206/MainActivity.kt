    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // val correct_username: String = "admin"
        ...
        btnlogin.setOnClickListener {
            ...
            val c = dbrw.rawQuery(if(ed_adname.length()<1)"SELECT * FROM myTable" else
                "SELECT * FROM myTable WHERE adname LIKE '${ed_adname.text}'",null)
            c.moveToFirst()

            //判斷是否輸入正確
            if((username.text.toString() == c.getString(1).toString())){
